...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cnt = 0
        identity = self.place
        while cnt < self.lower_bound:
            if not identity.is_hive:
                identity = identity.entrance
                cnt += 1
            else:
                return None
        while not identity.bees:
            if identity.entrance:
                identity = identity.entrance
                cnt += 1
            else:
                return None
        if cnt > self.upper_bound:
            return None
        if identity.bees:
            if not identity.is_hive:
                return random_bee(identity.bees)
        else:
            return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def __init__(self,cooldown=0,health=1):
        super().__init__(health)
    ...
    def action(self, gamestate):
        ...
        while cur:
            if isinstance(cur.ant,ContainerAnt):
                cur.ant.double()
                if cur.ant.ant_contained:
                    if not isinstance(cur.ant.ant_contained,FireAnt):
                        cur.ant.ant_contained.double()
            elif cur.ant:
                cur.ant.double()
            cur = cur.exit
        # END Problem 12
